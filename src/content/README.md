...
`src/content/content-tone.test.ts`.

## Content tone guidelines

Тексты в игре должны быть:

- **тёплыми** — обращение как к близкому человеку, без нажима;
- **рефлексивными** — приглашают заметить, а не объявляют истину;
- **дружелюбными к новичку** — без эзотерического жаргона без пояснения;
- **безоценочными** — клетка не «наказывает» и не «награждает»;
- **не пророчеством** — избегаем «судьба говорит», «это точно значит», «предсказывает»;
- **не диагнозом** — не «у тебя проблема», «диагноз», «ты всегда…»;
- **не заменой терапии** — при кризисе мягко перенаправляем к специалистам.
...
Проверка: `bun run content:check`. Скрипт запускает `content-tone` и
`content-integrity` тесты. Жёстко падает только на списке запрещённых
формулировок — мягкие предупреждения выводятся в консоль.
